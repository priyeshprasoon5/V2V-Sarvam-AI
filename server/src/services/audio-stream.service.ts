...
export class AudioStreamService {
   ...
   */
  public processLLMToken(sessionId: string, token: string, isFinal: boolean, languageCode: string = 'en-IN') {
   ...
   */
  private enqueueTTS(sessionId: string, text: string, languageCode: string, chunkIndex: number) {
   ...
   */
  private enqueueFinalEvents(sessionId: string, totalChunks: number) {
    let queue = this.ttsQueues.get(sessionId);
    if (!queue) return;
   ...
   */
  public cancelActiveStream(sessionId: string) {
    this.textBuffers.delete(sessionId);
    this.chunkIndices.delete(sessionId);
    this.ttsQueues.delete(sessionId);
  ...
  private sendEvent(socket: WebSocket, event: any) {
