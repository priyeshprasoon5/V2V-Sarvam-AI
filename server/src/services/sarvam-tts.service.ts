...
export class SarvamTTSService {
  ...
  private onAudioChunkCallback?: (sessionId: string, payload: string, chunkIndex: number) => void;
   ...
   */
  public onAudioChunk(callback: (sessionId: string, payload: string, chunkIndex: number) => void) {
  ...
  public async synthesizeText(
    ...
    try {
      ...
      const ttsLanguageMapping: Record<string, string> = {
        'mai-IN': 'hi-IN',
        'ur-IN': 'hi-IN'
      };
   ...
   */
  public cancelSynthesis(sessionId: string): void {
