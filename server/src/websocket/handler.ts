 ...
 */
export async function handleClientMessage(socket: WebSocket, event: ClientMessage): Promise<void> {
  ...
  switch (type) {
    ...
    case 'stt:start': {
      const startEvent = event as STTStartEvent;
      ...
      let sttService: SarvamSTTService = session.metadata.sttService;
      if (!sttService) {
      ...
      sttService.onFinal(async (transcript, detectedLanguage) => {
        ...
        try {
          ...
          llmService.onToken((sessId: string, token: string, isFinal: boolean) => {
    ...
    case 'stt:audio': {
      const audioEvent = event as STTAudioEvent;
      ...
      const sttService: SarvamSTTService = session.metadata?.sttService;
      if (sttService) {
    ...
    case 'audio:chunk': {
      const chunkEvent = event as AudioChunkEvent;
      ...
      console.log(
        `[WS Handler] [Session: ${sessionId}] Received Audio Chunk #${chunkIndex} | Size: ${chunkBuffer.length} bytes`
      );
 ...
 */
function sendResponse(socket: WebSocket, response: ServerMessage): void {
