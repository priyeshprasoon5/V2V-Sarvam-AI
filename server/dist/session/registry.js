...
export class SessionRegistry {
     ...
     */
    registerSession(sessionId, socket) {
     ...
     */
    setStreamingState(sessionId, isStreaming) {
        const session = this.sessions.get(sessionId);
        if (session) {
     ...
     */
    incrementChunkMetrics(sessionId, byteLength) {
        const session = this.sessions.get(sessionId);
        if (session) {
    ...
    removeSession(sessionId) {
        ...
        if (session) {
            ...
            if (sttService) {
                sttService.stopStream(sessionId).catch((err) => {
     ...
     */
    cleanupExpiredSessions(timeoutMs = 30 * 60 * 1000) {
        const now = Date.now();
